...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/01/09 15:03:29 by ebalsami          #+#    #+#             */
/*   Updated: 2022/01/09 15:03:30 by ebalsami         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	sem_unlink("fork");
	data->config.sem_fork = sem_open(
			"fork", O_CREAT, 0644, data->config.count_philo);
	sem_unlink("print");
	...
	if (data->config.sem_fork == SEM_FAILED
		|| data->config.sem_print == SEM_FAILED
		|| data->config.sem_dead == SEM_FAILED)
		return (1);
	return (0);
}
